...
export function StoryCard({ story, className }: { story: Story; className?: string }) {
  return (
    <motion.article
      ...
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-[28px] bg-white shadow-card ring-1 ring-hope/10",
        className,
      )}
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-serif text-xl font-semibold tracking-tight text-hope-deep">
          {story.title}
        </h3>
        <p className="mt-3 text-sm leading-relaxed text-hope-muted">{story.beforeAfter}</p>
